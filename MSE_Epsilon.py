...
def main():
    ...
    ##### Calculate MSE calperpoint times for each epsilon #####
    mseepsilons_start_time = time()
    print (f'\nMSE Calculation with varying epsilon started at {datetime.fromtimestamp(mseepsilons_start_time)}')
    N = multiprocessing.cpu_count()
    for epsilon in epsilons:
        mseepsilon_start_time = time()
        with multiprocessing.Pool(N-1) as pool:
            mseepsilon = list(ar.get() for ar in [pool.apply_async(rollout, args=(rawdata, maxdays, epsilon, allcordata, rawcorr[2])) for cal in range(calperpoint)])
            grr_mse_mean = mean(map(lambda x : x[0], mseepsilon))
            new_mse_mean = mean(map(lambda x : x[1], mseepsilon))
            grrmseepsilons.append(grr_mse_mean)
            newmseepsilons.append(new_mse_mean)
            pool.close()
            pool.join()
        print (f'MSE Calculation for epsilon {epsilon} last for {timedelta(seconds=(time() - mseepsilon_start_time))}', flush=True)   
    print (f'MSE Calculation with varying epsilon last for {timedelta(seconds=(time() - mseepsilons_start_time))}', flush=True)

    ##### Plot figures and save results in csv #####
    
    plt.plot(epsilons, grrmseepsilons, label='GRR MSE')
    plt.plot(epsilons, newmseepsilons, label='Proposed Method MSE')
    plt.xlabel('Epsilons')
    plt.ylabel('MSE')
    plt.legend()
    plt.savefig('epsnormal.png')

    df1 = pd.DataFrame(list(zip(epsilons, grrmseepsilons, newmseepsilons)), columns = ['Epsilon','GRR MSE', 'Proposed Method MSE'])
    df1.to_csv('epsnormal.csv', index=False)
