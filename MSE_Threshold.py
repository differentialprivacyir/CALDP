...
def main():
    grrmsethresholds = list()
    newmsethresholds = list()
    thresholds = np.arange(0.1, maxthreshold+0.1, 0.1)
    ...
    ##### Calculate MSE calperpoint times for each threshold #####
    msethresholds_start_time = time()
    print (f'\nMSE Calculation with varying threshold started at {datetime.fromtimestamp(msethresholds_start_time)}')
    N = multiprocessing.cpu_count()
    for threshold in thresholds:
        msethreshold_start_time = time()
        with multiprocessing.Pool(N) as pool:
            rawintercorrdata = list(map(lambda x : rawintercorr_pd.index[rawintercorr_pd[x] >= threshold].tolist() ,rawintercorr_pd))
            ...
            rawnextcorrdata = list(map(lambda x : rawnextdaycorr_pd.index[rawnextdaycorr_pd[x] >= threshold].tolist(),rawnextdaycorr_pd))
            ...
            msethreshold = list(ar.get() for ar in [pool.apply_async(rollout, args=(rawdata, maxdays, fixepsilon, allcordata, rawcorr[2])) for cal in range(calperpoint)])
            grr_mse_mean = mean(map(lambda x : x[0], msethreshold))
            new_mse_mean = mean(map(lambda x : x[1], msethreshold))
            grrmsethresholds.append(grr_mse_mean)
            newmsethresholds.append(new_mse_mean)
            pool.close()
            pool.join()
        print (f'MSE Calculation for threshold {threshold} last for {timedelta(seconds=(time() - msethreshold_start_time))}', flush=True)   
    print (f'MSE Calculation with varying threshold last for {timedelta(seconds=(time() - msethresholds_start_time))}', flush=True)

    ##### Plot figures and save results in csv #####
    plt.plot(thresholds, grrmsethresholds, label='GRR MSE')
    plt.plot(thresholds, newmsethresholds, label='Proposed Method MSE')
    plt.xlabel('Thresholds')
    plt.ylabel('MSE')
    plt.legend()
    plt.savefig('threshnormal.png')
    plt.close()         
    
    df1 = pd.DataFrame(list(zip(thresholds, grrmsethresholds, newmsethresholds)), columns = ['Threshold','GRR MSE', 'Proposed Method MSE'])
    df1.to_csv('threshnormal.csv', index=False)
